...
- **Split 60/20/20 por cantidad de CHUNKS, global sobre todo el corpus** (no por cantidad de documentos, no recalculado por proyecto): los documentos de este corpus varían mucho en tamaño (de 1 a más de 50 chunks), así que repartir "por cabeza" no ve ese peso — un documento de 51 chunks cuenta igual que uno de 1 chunk. La primera versión de este balanceo sí lo hacía por proyecto, pero eso tiene un problema real: cada proyecto, evaluado aislado, manda "razonablemente" su documento más pesado a la cuota más grande (train) — con 10 proyectos haciendo eso a la vez, sin coordinación entre ellos, train terminaba en ~74% del corpus real, muy lejos del 60% buscado. La solución es un algoritmo LPT (longest-processing-time-first, un heurístico clásico de balanceo de carga) que corre sobre TODOS los grupos de documentos del corpus a la vez: ordena por peso (en chunks) de mayor a menor, y cada uno se lo lleva el split que en ese momento esté más lejos de su cuota — con el estado compartido entre proyectos, el resultado real termina en 59.9/20.1/20.1% (antes: 46/11/43%). El costo de este cambio: ya no se garantiza que cada proyecto tenga representación en los 3 splits (antes sí, por diseño) — queda auditado con warnings explícitos en `split_report.json` cuando un proyecto pierde representación en val o test.
